...
<div align="center">
  <a href="docs/notebook.jpg">
    <img src="docs/notebook.jpg" alt="VS Code screenshot showing test-setup.ipynb notebook with project code integration. The notebook demonstrates importing from agent_course_google.placeholder and testing the add() function with successful output. Pink arrows point to the kernel selector in the top right, with 'select' annotation indicating where to choose the Python 3.13.5 (.venv) environment." width="800">
  </a>
...
```
agent-course-google/
├── src/agent_course_google/   # Package source code
...
2. Clone the repository to "my-project-name"
   ...
   ```bash
    git clone https://github.com/michellepace/agent-course-google.git my-project-name
   ```
...
3. Run these terminal commands for first-time setup:
    ...
    # Configure git for clean notebook commits (strips outputs, enables diffs)
...
1. Instruct AI (renaming): *Refactor this project to consistently rename it from "agent-course-google" to "my-new-project-name" throughout, use `git mv` for renaming!*
